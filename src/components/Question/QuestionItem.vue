<template>
    <div class="delete-page">
        <div class="delete-page__section">
            <div class="delete-page__image-svg">
                <svg fill="#CD5C5C" version="1.1" id="Ebene_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="50" height="50" viewBox="0 0 64 64" enable-background="new 0 0 64 64" xml:space="preserve"> <g> 
                    <path d="M17.586,46.414C17.977,46.805,18.488,47,19,47s1.023-0.195,1.414-0.586L32,34.828l11.586,11.586 C43.977,46.805,44.488,47,45,47s1.023-0.195,1.414-0.586c0.781-0.781,0.781-2.047,0-2.828L34.828,32l11.586-11.586 c0.781-0.781,0.781-2.047,0-2.828c-0.781-0.781-2.047-0.781-2.828,0L32,29.172L20.414,17.586c-0.781-0.781-2.047-0.781-2.828,0 c-0.781,0.781-0.781,2.047,0,2.828L29.172,32L17.586,43.586C16.805,44.367,16.805,45.633,17.586,46.414z"/> 
                    <path d="M32,64c8.547,0,16.583-3.329,22.626-9.373C60.671,48.583,64,40.547,64,32s-3.329-16.583-9.374-22.626 C48.583,3.329,40.547,0,32,0S15.417,3.329,9.374,9.373C3.329,15.417,0,23.453,0,32s3.329,16.583,9.374,22.626 C15.417,60.671,23.453,64,32,64z M12.202,12.202C17.49,6.913,24.521,4,32,4s14.51,2.913,19.798,8.202C57.087,17.49,60,24.521,60,32 s-2.913,14.51-8.202,19.798C46.51,57.087,39.479,60,32,60s-14.51-2.913-19.798-8.202C6.913,46.51,4,39.479,4,32 S6.913,17.49,12.202,12.202z"/> </g> 
                </svg>
            </div>
            <div class="delete-page__title">
                <h2>Are you sure?</h2>
                <small>Do you really want to delete your Profile? This process cannot be undone.</small>
            </div>
            <div class="delete-page__buttons">
                <button class="delete-page__button-delete" @click="deleteUser">Delete</button>
                <button class="delete-page__button-cancel" @click="$emit('close')">Cancel</button>
            </div>
        </div>
    </div>
</template>

<script>
import { useStore } from 'vuex'
export default {
    emits: ['close'],
    setup() {
        const store = useStore()
        const deleteUser =  async () => {
            await store.dispatch('deleteUser/remove')
        }
        return {
            deleteUser
        }
    }
}
</script>

<style lang="scss" scoped>
.delete-page {
    display: flex;
    justify-content: center;
    width: 100%;
    height: 100%;
    margin-top: 100px;
    // .delete-page__section
    
    
    
    &__image-svg {
        margin-bottom: 20px;
        svg {
            width: 80px;
            height: 80px;
        }
    }

    &__section {
        display: flex;
        align-items: center;
        flex-direction: column;
        background-color: #fff;
        padding: 30px 50px 0px 50px;
        border-radius: 10px;
        width: 40%;
        height: 40%;
    }

    // .delete-page__title

    &__title {
        display: flex;
        flex-direction: column;
        align-items: center;
        color: #000;
        gap: 10px;
        margin-bottom: 10px;
        small {
            opacity: 0.5;
        }
    }

    // .delete-page__button


    &__buttons {
        display: flex;
        justify-content: space-around;
        width: 100%;
    }

    // .delete-page__button-delete

    &__button-delete {
        background: #FA8072;
        padding: 10px 30px;
        border-radius: 10px;
    }

    // .delete-page__button-cancel

    &__button-cancel {
        background: #D8BFD8;
        padding: 10px 30px;
        border-radius: 10px;
    }
}
</style>